...
/**
 * Client-side token utilities using Supabase client
 * Works with RLS policies directly from browser
 */
...
export async function deductTokens(
  ...
  try {
    ...
    if (currentBalance < amount) {
      throw new Error(
        `Insufficient tokens. You have ${currentBalance} but need ${amount}`,
      );
    }
